...
def _convolve(kernel, in_img):
    ...
    if in_img.ndim > 2:
        ...
        img_out = np.zeros((mk+mi+1,nk+ni+1,num_channel))
        for k in range(num_channel):
            img_new = np.zeros((mk+2*mi-2,nk+2*ni-2))
            img_new[mi:mk+mi,ni:nk+ni] = in_img[:,:,k]
            for i in range(mk+mi+1):
                for j in range(nk+ni+1):
                    img_out[i,j,k] = np.sum(img_new[i:i+mk,j:j+nk]*kernel)
    else:
        mi,ni = in_img.shape
        num_channel = 1
        img_out = np.zeros((mk+mi+1,nk+ni+1))
        img_new = np.zeros((mk+2*mi-2,nk+2*ni-2))
        img_new[mk:mk+mi,nk:nk+ni] = in_img
        for i in range(mk+mi+1):
            for j in range(nk+ni+1):
                img_out[i,j] = np.sum(img_new[i:i+mk,j:j+nk]*kernel)
